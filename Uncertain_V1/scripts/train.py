...
parser.add_argument("--s_num", help="sample number", type=int, default=5)
parser.add_argument(
...
parser.add_argument(
    "-c_model", "--c_model", help="choose certain model",action="store_true"
)
parser.add_argument(
    "-uc_model", "--uc_model", help="choose uncertain model",action="store_true"
)
parser.add_argument(
    "--lambda_nll", help="NLL loss weight", type=float, default=0.1, dest="lambda_nll"
)
    ...
    for epoch in range(1, args.n_epochs + 1):
        ...
        for batch in it:
            ...
            for q, s, pid in zip(q, s, pid):
                ...
                loss,pred_loss,nl_loss,_ = model.get_loss(q, s, pid)
                ...
                total_nllloss+=nl_loss.item()
                # total_nllloss+=0
                total_pred_loss += pred_loss.item()
                total_cnt += 1  # (s >= 0).sum().item()

                postfix = {"loss": total_loss / total_cnt}
                postfix["nll_loss"] = total_nllloss / total_cnt
                ...
                #     postfix["reg_loss"] = total_reg_loss / total_cnt
        ...
        with torch.no_grad():
            ...
            for batch in it:
                ...
                for q, s, pid in zip(q, s, pid):
                    ...
                    if pid is not None:
                        pid = pid.to(args.device)
                    if args.model == "DTransformer":    
                        y,logits_mean, *_ = model.predict(q, s, pid)
                    else:
                        q_emb, s_emb, lens, p_diff = model.embedding(q, s, pid)
                        y,logits_mean, *_ = model.predict(q_emb, s_emb,lens,p_diff,pid)
                    
                    evaluator.evaluate(s, torch.sigmoid(logits_mean))
        ...
        if r["auc"] > best["auc"]:
            ...
            if args.c_model:
                model_path = os.path.join(
                    args.output_dir, "best_model_c.pt"
                )
                print("saving snapshot to:", model_path)
                torch.save(model.state_dict(), model_path)
            elif args.uc_model:
                model_path = os.path.join(
                    args.output_dir, "best_model_uc.pt"
                )
                print("saving snapshot to:", model_path)
                torch.save(model.state_dict(), model_path)
